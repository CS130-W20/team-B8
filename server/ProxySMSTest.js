...
setTimeout(function(){
    ...
    assert.equal(Object.keys(SMS1.event).length, 0);
    ...
    var event4 = {
        ...
        locationName: "Royce",
        date: "03-20-2020",
        host: {name: "Sean Derman", phone: "1234567890"}
    }

    var SMS4 = new ProxySMSMessage(request4, event4);
    SMS4.formatMessage();
    assert.equal(SMS4.message, `You have just received a text on BruinMeet from ${event4.host.name}, in relation to the event ${event4.title} at ${event4.locationName} during ${event4.date}\r\n\r\n${request4.message}\r\n\r\nFeel free to message ${event4.host.name} at ${event4.host.phone}`);
